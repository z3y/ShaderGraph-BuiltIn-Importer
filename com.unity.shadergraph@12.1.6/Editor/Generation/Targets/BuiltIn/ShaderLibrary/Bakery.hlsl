...
void BakerySHLightmapAndSpecular(inout half3 lightMap, float2 lightmapUV, inout half3 directSpecular, float3 normalWS, float3 viewDir, half roughness)
{
        ...
        half3 L0 = lightMap;
        float3 nL1x = _RNM0.Sample(bakery_bilinear_clamp_sampler, lightmapUV) * 2.0 - 1.0;
        float3 nL1y = _RNM1.Sample(bakery_bilinear_clamp_sampler, lightmapUV) * 2.0 - 1.0;
        float3 nL1z = _RNM2.Sample(bakery_bilinear_clamp_sampler, lightmapUV) * 2.0 - 1.0;
